...
const SHEET_STYLE = `
  ...
  *{box-sizing:border-box}
  ...
  /* Ship to, inside the vendor box. The rule closes the VENDOR off -- it sits
     after that party's last field (GSTIN) -- and the SHIP TO heading carries no
     bar of its own: two rules two millimetres apart read as a mistake. */
  .kv-end{border-bottom:1px solid var(--paper-rule-soft);margin:0 0 2.5mm;padding:0}
  .bt2{margin-top:0;padding-bottom:0;border-bottom:none}
...
function boxHtml(
  ...
  // Rule first, closing the vendor's fields; then the second party's heading,
  // which draws no bar of its own.
  return (
    head +
    '<div class="kv-end"></div>' +
