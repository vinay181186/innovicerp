-- 0125 — a QC (or qc_required) op reads `complete` only when its pieces are
-- ACCEPTED, not merely inspected.
--
-- THE BUG: the completion gate's QC test was
--   qc_accepted_qty + qc_rejected_qty >= <input>
-- i.e. an op counted as finished once every piece had been *inspected*, whether
-- passed or failed. So an op that rejected everything (accepted 0, rejected 10)
-- read `complete`, and a rework child that itself rejected all its pieces read
-- `complete` too — both the per-op and, through v_jc_status, the overall JC
-- status showed Complete while zero good pieces existed. Observed on the test
-- site: IN-JC-26-00023 and its child IN-JC-26-00023-RW1 (Op-2 QC rejected 10,
-- accepted 0) both read Complete; on production the same fault left
-- IN-JC-26-00009 reading Complete with 5 of 9 accepted.
--
-- THE RULE the user stated: an op is complete only when all its qty is DONE and
-- ACCEPTED; the last op accepted makes the JC overall complete; a parent JC is
-- complete only once its rework children have actually recovered their pieces.
--
-- FIX (one token): the QC acceptance test becomes
--   qc_accepted_qty >= <input>
-- Rejected pieces no longer satisfy completion. This is self-healing across the
-- recovery lifecycle: rework/repair pieces that are recovered re-enter the
-- origin op as a qc-accepted op_log (recovery.ts reinjectIntoOriginOp), so
-- qc_accepted rises to the bar and the op then completes; pieces that are
-- scrapped or stay rejected never do, so the op holds at `qc_pending` /
-- `in_progress` until they are genuinely recovered or the JC is closed by hand
-- (v_jc_status still reads `closed` from job_cards.closed_at). The rwc / rw /
-- rtv gates from 0124/0093/0089 are unchanged.
--
-- SCOPE: this is the ONLY change from the 0124 view — the FIRST `complete`
-- branch's QC clause. The column list, order and types are byte-for-byte
-- identical, so CREATE OR REPLACE is legal and the dependent v_jc_status (which
-- reads only computed_status) is unaffected. Idempotent; apply to BOTH the
