...
/**
 * Read-only document traceability for one plan (new-ERP navigation aid, not in
 * legacy). Mirrors getSalesOrderRelated: single withUserContext transaction so
 * RLS applies, an existence/visibility check, and every subquery company-scoped
 * + soft-delete filtered.
 *
 * Upstream (what this plan was built FROM):
 *   - plans.so_line_id      → sales_order_lines → sales_order (SO the plan lines up under)
 *   - plans.jw_line_id      → job_work_order_lines → job_work_order
 *   - plans.bom_master_id   → bom_masters
 *   - plans.item_id         → items (master)
 *
 * Downstream (generated by executePlan):
 *   - plans.jc_id           → job_cards
 *   - DISTINCT purchase_requests referenced by plans.dp_pr_id / fo_pr_id /
  ...
  return withUserContext(user, async (tx) => {
    const headers = await tx
      .select({
        ...
        jwLineId: plans.jwLineId,
        bomMasterId: plans.bomMasterId,
        itemId: plans.itemId,
        jcId: plans.jcId,
        dpPrId: plans.dpPrId,
        foPrId: plans.foPrId,
        foMatPrId: plans.foMatPrId,
    ...
    const soRows = header.soLineId
      ? await tx
          ...
          .where(
            and(
              eq(salesOrderLines.id, header.soLineId),
    ...
    // ── Upstream: source Job Work Order (via JW line → header) ───────────────
    const jwoRows = header.jwLineId
      ? await tx
          .select({
            id: jobWorkOrders.id,
          ...
          .where(
            and(
              eq(jobWorkOrderLines.id, header.jwLineId),
    ...
    // ── Upstream: BOM master ─────────────────────────────────────────────────
    const bomRows = header.bomMasterId
      ? await tx
          .select({
            id: bomMasters.id,
          ...
          .where(
            and(
              eq(bomMasters.id, header.bomMasterId),
    ...
    // ── Upstream: item under plan (master) ───────────────────────────────────
    ...
    // ── Downstream: generated Job Card ───────────────────────────────────────
    const jcRows = header.jcId
      ? await tx
          .select({ id: jobCards.id, code: jobCards.code, date: jobCards.jcDate })
          .from(jobCards)
          .where(
            and(
              eq(jobCards.id, header.jcId),
    ...
    // ── Downstream: purchase requests referenced by this plan + its ops ──────
    const opPrRows = await tx
    ...
    const jwoSection = section(
      'job-work-order',
      'Job Work Order',
      '🛠',
      'job-work-order',
      jwoRows.map((r) => row(r.id, r.code, r.status, r.date)),
    );
    ...
    const jcSection = section(
      ...
      '📋',
      'job-card',
      jcRows.map((r) => row(r.id, r.code, null, r.date)),
    );
    ...
    const upstream = [soSection, jwoSection, bomSection, itemSection];
    const downstream = [jcSection, prSection];
    return {
      self: { module: 'plans', code: header.code },
      upstream,
      downstream,
      related: [],
      timeline: buildTimeline(
        {
          ts: toIsoDate(header.planDate),
          label: 'Plan created',
          code: header.code,
          routeKind: 'plan',
          linkId: id,
