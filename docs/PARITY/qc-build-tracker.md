...
- [~] **3. QC History** — BACKEND done (qc-history shared shape + service expose `clientPoLineNo`/`qcCallDate`/`logNo`) + `qc-alert-blink` keyframe in innovic-theme.css + `qc-history/lib/export.ts` (xlsx). **WEB UI PENDING**: apply link-fix 🔬→`/qc-call-register`, overdue row-blink class, Export buttons, IST dates in `qc-history/routes/index.tsx`. Report col → chunk 2.
- [~] **4. QC Call Register** — BACKEND ready (qc-history fields above). **WEB UI PENDING** in `qc-call-register/routes/index.tsx`: `[CPO:xxx]` tag, logNo, Called→waiting→Response via qcCallDate, COMPLETE count, per-panel search, overdue blink, inspector datalist. 📎 attach report → chunk 2.
