...
- **P2 — Template-consuming prints (B): ✅ DONE 2026-05-27.** Shared `apps/web/src/lib/print/doc-print.ts` (generalised P1 builder → real line items + meta + PO tax totals + amount-in-words) + `lib/print/company.ts`. P1 `test-print.ts` refactored to delegate. Per-doc assemblers `print-po.ts` / `print-ospdc.ts` / `print-jwdc.ts`. Print buttons on PO detail + OSP DC (delivery-challans) detail + NEW `jw-dc/routes/detail.tsx`. Vendor + company fetched client-side at print (no endpoint change). DELTA: companies row has no email column → no E-Mail line on real prints (legacy hardcoded gmail). 8/8 web unit tests + typecheck + lint clean.
- **P3 — Fixed-layout prints (C): ✅ DONE 2026-05-27 (6/7), commits `6ef78a2` + `3338472`.** Shared `apps/web/src/lib/print/print-window.ts` (`printWindow({title,body,company})`, legacy L10524 stylesheet) + per-doc builders in each module's `lib/`: ✅ Route Card (L10629, route-card detail), ✅ Machine Queue (L10661, machine-loading board), ✅ Daily Report (L10918, daily-report page), ✅ Dispatch Register (L10789, delivery-challans list — header-grain), ✅ Drawing (L10490, item detail — embeds signed-URL img/pdf), ✅ Job Card (L10582, job-cards list row — lazily fetches `useJcOpsEnriched`; DELTA: Drawing No./Material/Program/Tool No./Production-Log not in current reads). **⏸ Invoice (`_printInvoice` L21314) DEFERRED — no React invoice/tax-invoice module exists (dispatch is via delivery-challans).** web typecheck + lint clean.

> **PRINT TEMPLATES FEATURE COMPLETE** — P1 + P2 + P3(6/7). Only the Invoice print remains, gated on a future invoice module.

- **Follow-up — Service PO print (AUDIT-5, 2026-06-01):** added a 4th template doc type `SERVICE PO` (key prefix `spo_`, 5 blocks + defaults + vars) to the print-templates schema — code-only, no migration (templates seed lazily; `print_templates.template_key` has no enumerating CHECK). `doc-print.ts` treats `SERVICE PO` like `PO` (priced goods table + totals + amount-in-words); `DOC_TITLE` + editor `DOC_COLOR`/`DOC_LABEL` extended. New `service-pos/lib/print-spo.ts` + Print button on the Service PO detail page (mirrors `print-po.ts`). Closes the PUR-1/ADR-037 deferred "Service PO print template (Phase F slot)". Shared print-template test bumped 15→20 keys.
