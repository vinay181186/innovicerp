...
> Last updated: 2026-05-02 (T-035a done — Phase 5 procurement schema design approved via ADR-015 (5 new tables + jc_ops outsource FK upgrade). T-035b (Drizzle schema + migrations) is next.)
...
| T-035b | Phase 5 Drizzle schema + migrations to dev Supabase (4 migration files; jc_ops ALTER) | [ ] Active |
| T-035c | Phase 5 transform + load + validate-phase5 (incl. jc_ops outsource_*_id backfill from legacy text) | [ ] |
