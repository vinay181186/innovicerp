...
| Tasks              | tasks, task_comments, task_history, daily_reports, daily_report_lines                   | tasks, taskComments, taskHistory, dailyReports, dailyReportLines (Task Board + Daily Task Reports, migrations 0051 + 0139) |
...
| `payment_days`             | `integer`       | nullable, CHECK 0..365. Payment Days — days allowed to pay an invoice; default for a new invoice's Payment Terms (0150, ADR-188) |
...
| `machine_group_id`   | `uuid`          | nullable, FK -> `machine_groups.id` (migration 0116). The Machine Group picker on the machine form. **Additive** — sits alongside `machine_type`, does not replace it |
| `product_code`       | `text`          | nullable (migration 0116). The product this machine runs. Free text, typed on the machine form |
...
Indexes: `unique (company_id, code) where deleted_at is null`, `(company_id) where deleted_at is null`, `(company_id, status) where deleted_at is null` (for the live operations board, Phase 3+). Migration 0116 adds `(machine_group_id) where deleted_at is null`.

### `machine_groups`

Machine GROUP master (migration 0116). The second tab of the Machine Master screen: the groups the shop floor actually runs — `VMC`, `CNC`, `Lathe`, `Grinding`. Shaped as a deliberate sibling of `material_grades` and `tpi_masters`, including the permanent-code rule.

**Additive, not a replacement.** `machines.machine_type` stays exactly as it was — a free-text field the user still types on the machine form — and the six modules that read it (alerts AL-013, job-queue, machine-loading, production-schedule, shop-floor) are untouched. The group is a second, independent field beside it. Migration 0116 seeds this master from the distinct `machine_type` values already in use purely so the picker opens with real groups rather than empty; the two fields are independent from then on.

There is no separate `name` column, for the same reason `tpi_masters` has none: the group is one word, so one field, with nothing to keep in sync.

| Column               | Type            | Notes                                                                                                                                             |
| -------------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `id`                 | `uuid`          | PK                                                                                                                                                |
| `company_id`         | `uuid`          | not null, FK                                                                                                                                      |
| `code`               | `text`          | not null. The group as written on the shop floor — `VMC`. Business key, and **permanent once created**: `updateMachineGroupInputSchema` omits it. Retire with `is_active` instead of renaming |
| `description`        | `text`          | nullable. What the group covers, which shop it lives in                                                                                           |
| `is_active`          | `boolean`       | not null, default `true`. Only active groups are offered on the machine form                                                                       |
| audit + `deleted_at` | (audit pattern) |                                                                                                                                                   |

Indexes:

- `unique (company_id, code) where deleted_at is null`
- `(company_id, is_active) where deleted_at is null` — drives the "active groups only" picker

RLS:

- `machine_groups_company_read` (any role)
- `machine_groups_manager_write` (admin/manager only)

Access control: form key `machine_create` — the SAME key as `machines`, because Machines and Machine Groups are two tabs of one screen in the user's head. Create needs `entry`, edit needs `edit`, soft-delete needs `edit` **and** `approve`.

Soft-deleting a group that a live machine still points at is refused with a `ConflictError` naming how many machines use it. No cascade, and the machines' `machine_group_id` is never cleared behind the user's back.
...
| `current_revision`   | `integer`       | not null, default `0` (was `1` until `0111_route_card_revision_starts_at_zero.sql`). A card is **born at Rev 0**; the service layer bumps it by one each time the ops change, so the first edit makes it Rev 1. The six cards created before that migration kept their original numbering (`IN-RC-00001` at 3, the rest at 1) — deliberately not backfilled |
| `notes`              | `text`          | nullable                                                       |
| audit + `deleted_at` | (audit pattern) |                                                                |
...
`v_osp_wip.ready_to_send_qty` (0110, 2026-09-05) — how many pieces may go to the OSP vendor **right now**: `GREATEST(0, v_jc_op_status.input_avail - in-house completed on this op - jc_ops.outsource_sent_qty)`. Every other qty on this view is anchored to `job_cards.order_qty`, so `not_sent_qty` (order − sent) is an ORDER-level figure that over-states what can ship: IN-JC-26-00008 op 8 read `not_sent 70` while op 7 had cleared only 30, all 30 already at the vendor — true answer 0. `not_sent_qty` is deliberately left unchanged (it correctly answers "how much of this order still has to be processed"); the two sit side by side on the register. The formula mirrors `applyOutwardToJcOp` in `delivery-challans/cascades.ts` line for line, so the register shows the number the challan will accept. **One deliberate divergence:** the client-material gate (`loadMaterialCap` — a JWSO job card whose first op waits on the client's material) is NOT subtracted, since it needs per-op party-material lookups that do not belong in a register view and only ever LOWERS the cap — so the challan may still refuse a qty this column offered. The view now joins `v_jc_op_status`. Surfaced as a column + KPI tile + a fourth `ready_to_send` filter value on the OSP At-Vendor Register. See ADR-141.
...
| `drawing_no`         | `text`          | nullable                                                                        |
| `revision`           | `integer`       | not null, default `0`. The DRAWING FILE's revision number, owned by the server — born at 0, +1 only when `drawing_file_path` actually changes (replaced or cleared), never on a plain re-save. Free text until `0112_so_line_drawing_revisions.sql` cast it (the only three live values were `'1'`, so the cast was lossless). Every step archived in `so_line_drawing_revisions` |
| `drawing_file_path`  | `text`          | nullable. Storage path of the uploaded drawing (qc-docs bucket, folder `so-line-drawings`); viewed via a short-lived signed URL. Added `0104_so_line_drawing_revision.sql` |
| `uom`                | `uom`           | not null, default `'NOS'`                                                       |
| `order_qty`          | `integer`       | not null, check `> 0`                                                           |
...
| `balance_closed_at`  | `timestamptz`   | nullable (0117). Set when the buyer SHORT-CLOSES the remainder — "we ordered 10 of 100 and the rest is not coming". While set, `balanceQty` reports 0 and the PR leaves the PO picker |
| `balance_closed_by`  | `uuid`          | nullable, FK -> users(id) (0117)                                                                                                   |
| `balance_closed_reason` | `text`       | nullable (0117). Required by CHECK `purchase_requests_balance_close_complete` whenever `balance_closed_at` is set — an unexplained abandonment is the one thing nobody can answer later |
| audit + `deleted_at` | (audit pattern) |                                                                                                                                    |
...
Which purchase order lines cover an OUTSOURCED operation (migration 0118 / ADR-152 phase 4). One row per (op, PO line) with the quantity that link carries, so an operation's outsourced quantity is the SUM of its live links.

`jc_ops.outsource_po_line_id` is a single column and was the join key for the whole outsource chain — the outward-DC quantity guard, the receipt cascade and the job card's OSP trail all matched on it. Once a Purchase Request could be covered by several POs (ADR-152 phases 1-2), an outsourced op could only ever follow ONE of them: the second PO's challan and receipt never cascaded back, so material sat at the vendor and the operation stayed at `sent` forever with no error anywhere.

Same shape as the ADR-081 outsource-balance dual lane — an op split across lanes and recombined — applied to purchasing instead of to in-house vs OSP.

`jc_ops.outsource_po_line_id` is deliberately **KEPT** and still written with the FIRST link, exactly as `machines.machine_type` was kept in 0116. Six further readers (`goods-receipt-notes/cascades.ts`, `jc-ops/service.ts`, `customer-dispatches`, `jw-returns`, `op-entry/sales-cascade.ts`, `so-costing`) still use the column and keep working unchanged; on a split operation they see only the first PO. Moving them is the natural phase 5.
...
| `company_id`             | `uuid`          | not null, FK                                                                                   |
| `jc_op_id`               | `uuid`          | not null, FK -> jc_ops(id) ON DELETE CASCADE                                                    |
| `purchase_order_line_id` | `uuid`          | not null, FK -> purchase_order_lines(id) ON DELETE CASCADE                                      |
| `qty`                    | `integer`       | not null, CHECK > 0. How much of the operation this line covers. A link carrying nothing is a deleted link, not a zero one |
| audit + `deleted_at`     | (audit pattern) |                                                                                                |

Indexes:

- `unique (jc_op_id, purchase_order_line_id) where deleted_at is null` — one link per pair; a second PO is a different LINE, not a duplicate row
- `(purchase_order_line_id) where deleted_at is null` — challan/receipt arrives holding a PO line and needs its ops
- `(jc_op_id) where deleted_at is null` — the job card holds an op and needs its lines

RLS:

- `jc_op_po_lines_company_read` (any role)
- `jc_op_po_lines_manager_write` (admin/manager only)

Backfill (0118): every op already pointing at a PO line became one link carrying that line's quantity, so the table is a superset of the old column and a caller can be moved across without a behaviour change.
...
Master-data lookup of the third-party inspectors this company works with (migration 0114 / ADR-146). Drives the Inspector dropdown on the TPI screen. Deliberately shaped as a sibling of `qc_processes` one screen away in the Quality menu, including the permanent-code rule.

`op_log.tpi_inspector` is **NOT** FK'd to this table and never will be — it stays a text snapshot of the name as it stood when the inspection was signed. A QC log is a record of what was true on the day; retiring or correcting a master row must not rewrite an inspection someone already put their name to. See ADR-146.

| Column               | Type            | Notes                                                                                                                                                                    |
| -------------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `id`                 | `uuid`          | PK                                                                                                                                                                       |
| `company_id`         | `uuid`          | not null, FK                                                                                                                                                             |
| `code`               | `text`          | not null. The inspector's NAME — what the user types, reads and picks; labelled "Inspector Name" throughout the UI. Business key, and **permanent once created**: `updateTpiMasterInputSchema` omits it and the service never writes it, because every TPI log has already snapshotted it. Retire with `is_active` instead of renaming |
| `organization`       | `text`          | nullable. Who they inspect for (e.g. `L&T QA Department`). Rendered after the name in the dropdown, and auto-filled into the TPI form's Organization box on pick          |
| `contact_no`         | `text`          | nullable                                                                                                                                                                 |
| `email`              | `text`          | nullable. Format validated by the shared Zod schema; a blank submission is stored as null, not `''`                                                                       |
| `remarks`            | `text`          | nullable                                                                                                                                                                 |
| `is_active`          | `boolean`       | not null, default `true`. Only active inspectors are offered in the TPI dropdown                                                                                          |
| audit + `deleted_at` | (audit pattern) |                                                                                                                                                                          |

Indexes:

- `unique (company_id, code) where deleted_at is null`
- `(company_id, is_active) where deleted_at is null` — drives the "active inspectors only" dropdown

RLS:

- `tpi_masters_company_read` (any role)
- `tpi_masters_manager_write` (admin/manager only)

Access control: form key `tpimaster_create` (dept `qc`). Create needs `entry`, edit needs `edit`, soft-delete needs `edit` **and** `approve` — the same gate `qc_processes` uses.
...
| 2026-09-02 | `0102_dc_vehicle_no.sql` (hand-written, idempotent) | **A delivery challan records the vehicle, not just the transporter.** `delivery_challans.vehicle_no text` — nullable. Same column name and type as the four tables that already carried one (`jw_dc_outward`, `jw_dc_inward`, `jw_return_challans`, `customer_dispatches`); the OSP DC print and the gate register need the transporter's NAME and the vehicle NUMBER kept apart, and the print had been faking it by printing `transport` under a "Vehicle" label. |
| 2026-09-02 | `0103_po_line_source_pr.sql` (hand-written, idempotent) | **One PO may cover several Purchase Requests, so the PR link moves onto the line.** `purchase_order_lines.source_pr_id uuid REFERENCES purchase_requests(id) ON DELETE SET NULL` + partial index `purchase_order_lines_source_pr_idx` on non-null values where `deleted_at is null`, and `purchase_order_lines.ram_remark text`. The header's single `purchase_orders.pr_id` cannot represent the link once a PO spans more than one PR — the batch path already nulled it in that case — so "which PR did this line come from" was unanswerable for anything but OSP lines, which could be traced the long way round through `jc_ops`. `POST /purchase-orders` reopened on this basis: ADR-138 ("a PO is always raised against a Purchase Request") now holds at line level, enforced by the create input requiring at least one line to carry a `sourcePrId`. |
| 2026-09-02 | `0104_plan_op_tool_no.sql` (hand-written, idempotent) | **The plan stops losing the route card's tool number.** `plan_ops.tool_no text` — nullable. `route_card_ops` and `jc_ops` both carried a tool number and the plan sitting between them did not, so it was dropped on the way to the Job Card — and once plan execution began writing the route card back (the ADR-051 auto-save follow-up), executing a plan also **blanked a tool number the route card already held**. Silent loss in both directions. `program`, `tool_details` and `machine_id` were already carried correctly by the server; `outsource_vendor_id` was dropped on plan edit and is now preserved, but only while the op still names the same vendor in text — restoring it unconditionally would raise the outsourcing PR against the previous vendor, since execute prefers the id over the text. |
| 2026-09-03 | `0104_so_line_drawing_revision.sql` (hand-written, idempotent) | **A Sales Order LINE carries an optional drawing revision and an uploaded drawing document.** `sales_order_lines.revision text` (nullable — free-text revision, e.g. `A`/`R2`) and `sales_order_lines.drawing_file_path text` (nullable — storage path in the existing `qc-docs` bucket, folder `so-line-drawings`; uploaded direct from the web form, viewed via a short-lived signed URL). No new bucket or storage RLS — reuses the item-drawing / QC-doc storage path. Columns inherit `sales_order_lines`' existing RLS. **Applied to the shared DB 2026-09-03** (verified: both columns present, text, nullable). |
| 2026-09-05 | `0111_route_card_revision_starts_at_zero.sql` (hand-written, idempotent) | **A route card is born at revision 0, not 1.** `alter table route_cards alter column current_revision set default 0`. The number a card is created with lived in three places — the column default plus the two create paths in `route-cards/service.ts` (`createRouteCard` for the manual screen, `saveRouteCardForItem` for the Job Card / plan-execute auto-save) — and each wrote `1`, which made the first *edit* Rev 2. All three now write `0`, so the first edit is Rev 1. Three Zod validators had to be relaxed from `.positive()` to `.nonnegative()` or a freshly created card would have failed validation on the way out: `routeCardSchema.currentRevision`, `routeCardRevisionSchema.revisionNo` and `defaultRouteOpsResponseSchema.routeCardRevision` (the planning screen's Rev chip). **No backfill** — existing cards keep the numbers they were created with, on the user's instruction, so old and new numbering both exist by design. **Applied to the shared DB 2026-09-05** (verified: default now `0`, all 6 existing rows unchanged). |
| 2026-09-06 | `0112_so_line_drawing_revisions.sql` (hand-written, idempotent) | **The Rev on an SO line becomes the DRAWING FILE's revision number, with a history behind it.** `sales_order_lines.revision` goes `text` → `integer not null default 0` (the cast is lossless: of 42 live lines only 3 carried a value and all three were `'1'`). New append-only child table **`so_line_drawing_revisions`** — `id, company_id, sales_order_id, so_line_id, revision_no, action ('added'\|'replaced'\|'removed'), drawing_file_path (null on a removal), drawing_no, item_code_text, created_at, created_by`; unique on `(so_line_id, revision_no)`, indexed on `sales_order_id` and `(so_line_id, created_at)`, RLS company-read + manager-insert. A line is **born at Rev 0** and climbs by one only when `drawing_file_path` genuinely changes — the same "born at zero" rule as route cards (0111). **Seeded**: every line already holding a drawing gets one row at the number it currently carries (0 for the two with no Rev, 1 for the three that had `'1'`), stamped with that line's own creator and creation time; earlier drawings cannot be reconstructed so the trail starts where the record does. Read back via `GET /sales-orders/:id/drawing-history`. Also fixed en route: clearing a drawing had never persisted, because the form sent `undefined` and `JSON.stringify` dropped the key — the input schema is now `.nullable()` and the form posts an explicit `null`. **Applied to the shared DB 2026-09-06** — verified live: `revision` is `integer NOT NULL DEFAULT 0`, no NULLs remain (39 lines at 0, 3 at 1), 5 seeded history rows (IN-SO-00521 and 00523 at Rev 1, IN-SO-00526/527/528 at Rev 0, all `added`), RLS on with both policies and all 4 indexes present. |
| 2026-09-07 | `0117_pr_short_close.sql` (hand-written, idempotent) | **A Purchase Request can be SHORT-CLOSED** (ADR-152 phase 2). Three nullable columns on `purchase_requests`: `balance_closed_at`, `balance_closed_by`, `balance_closed_reason`; CHECK `purchase_requests_balance_close_complete` (a closed balance must carry a closer AND a non-blank reason); partial index `(company_id) where deleted_at is null and balance_closed_at is null`. Fixes the case where a part-ordered PR was immortal — it could not be edited (qty already committed to a vendor) nor rejected (both guards refused once any PO existed), so the unordered remainder sat in the "still to buy" list forever. Short-closing is deliberately NOT editing qty down: the PR still records that 100 was asked for and 10 bought. **ADDITIVE ONLY.** **NOT YET APPLIED** to test or production. |
| 2026-09-07 | `0118_jc_op_po_lines.sql` (hand-written, idempotent) | **An outsourced operation can sit on SEVERAL purchase order lines** (ADR-152 phase 4). New table `jc_op_po_lines` (see above) with backfill from `jc_ops.outsource_po_line_id`. Removes the single-column bottleneck that made the second PO of a split OSP request invisible to the outward-DC guard, the receipt cascade and the job card trail. `outsource_po_line_id` is KEPT and still written with the first link. **ADDITIVE ONLY.** **NOT YET APPLIED** to test or production. **Deploy order: migration FIRST, code second** — the six moved read sites resolve only through this table. |
| 2026-09-12 | `0122_qc_nc_handling.sql` (hand-written, idempotent) | **QC–NC handling per the Innovic QC–NC Handling Procedure R2** (ADR-161, design in `docs/QC-NC-HANDLING-DESIGN.md`). Enums: `nc_disposition` + `repair`; `nc_status` + `under_rework`, `under_repair`, `sent_to_vendor`, `received_qc_pending`. `nc_register` gains the quantity ledger and links: `qc_log_id → op_log`, `grn_line_id → goods_receipt_note_lines`, `split_from_nc_id → nc_register`, `child_job_card_id → job_cards`, `delivery_challan_id → delivery_challans`, `rtv_sent_qty`, `rtv_received_qty`, `cleared_qty`, `failed_qty` (numeric, default 0), `closed_at`, `closed_by`; CHECKs `cleared+failed <= rejected` and `rtv_received <= rtv_sent`; partial indexes on the three link columns. `job_cards` gains `parent_job_card_id → job_cards`, `origin_op_seq`, `recovery_kind` (CHECK in rework/repair) + partial index. `delivery_challans` gains `nc_id`, `job_card_id`, `reason`; `goods_receipt_notes` gains `nc_id` (replacement receipt); partial indexes on both `nc_id`s. `v_jc_op_status` re-emitted from 0093 with ONE additive term: an outsource op's output adds `op_log` qc qty, so pieces recovered after an outsource reject can re-enter that op (no such rows existed before, so today's numbers are unchanged; column list/order unchanged). New view `v_nc_op_breakup` per `jc_op_id`: `nc_raised_qty, under_rework_qty, under_repair_qty, sent_to_vendor_qty, received_qc_pending_qty, scrap_qty, nc_closed_qty, nc_open_qty, open_nc_count` — the §6 quantity breakup. **ADDITIVE ONLY**: nothing dropped, renamed or reordered; legacy in-route rework rows (`rework_op_seq` set) keep working. **Applied to the TEST database 2026-09-12** (21/21 statements). **NOT applied to production** — awaiting the user's word. |
| 2026-09-12 | `0123_route_card_plan_type.sql` (hand-written, idempotent) | **Route Card carries a Plan Type.** `route_cards.plan_type public.plan_type NOT NULL DEFAULT 'manufacture'` — the same three-way choice SO Planning asks (`manufacture` / `full_outsource` / `direct_purchase`), recorded once on the route card as the item's default. Existing rows read `manufacture`. Companion UI change: the ops table's `Type` dropdown is gone (row kind comes from the Add Op / OSP / QC button and is shown as a badge) and a **Machine Group** picker narrows the machine list per row, exactly as SO Planning does; group is not stored — it is read off the machine master. Applied to BOTH databases 2026-09-12. |
| 2026-09-07 | `0116_machine_groups.sql` (hand-written, idempotent) | **Machine Group master, plus `product_code` on machines.** One new table `machine_groups`: `id, company_id, code (the group as written on the shop floor — 'VMC' — unique per company, permanent once created), description, is_active`, audit + `deleted_at`; unique `(company_id, code) where deleted_at is null`, index `(company_id, is_active) where deleted_at is null`, RLS company-read + manager-write. Two new nullable columns on `machines`: `machine_group_id` (FK) and `product_code`, plus index `(machine_group_id) where deleted_at is null`. **ADDITIVE ONLY** — `machine_type` is deliberately kept as the free-text field it has always been (the user reversed an initial decision to remove it), so alerts AL-013, job-queue, machine-loading, production-schedule and shop-floor are all untouched; the group is a SECOND, independent field beside it. The backfill seeds the master from the distinct `machine_type` values already in use, purely so the picker opens with real groups instead of empty. Reuses the existing access key `machine_create` — Machines and Machine Groups are two tabs of one screen. **Applied to the TEST database 2026-09-07** (table present, both machine columns present, both RLS policies present). **NOT applied to production** — awaiting the user's word. |
| 2026-09-06 | `0114_tpi_masters.sql` (hand-written, idempotent) | **TPI Master — the third-party inspectors this company works with** (ADR-146). One new table `tpi_masters`: `id, company_id, code (the inspector's NAME, unique per company, permanent once created), organization, contact_no, email, remarks, is_active`, audit + `deleted_at`; unique `(company_id, code) where deleted_at is null`, index `(company_id, is_active) where deleted_at is null`, RLS company-read + manager-write. Shaped as a deliberate sibling of `qc_processes` — same columns, same permanent-code asymmetry (`code` omitted from the update schema). **ADDITIVE ONLY**: no existing row is touched and `op_log.tpi_inspector` deliberately stays a text snapshot rather than becoming an FK, because a QC log records what was true on the day it was signed. New access-control key `tpimaster_create` (dept `qc`). **Applied to the shared DB 2026-09-06** — verified live: all 13 columns present with the right types, 0 rows, RLS on with both policies, all 3 indexes created, and `op_log`'s three tpi_* columns unchanged (still `text`). |
|
| 2026-09-06 | `0115_qc_user_links.sql` (hand-written, idempotent) | **A QC entry links to the QC user who signed it off** (ADR-149, finishing ADR-147). One new nullable column `op_log.qc_user_id uuid REFERENCES users(id)` plus a partial index `(qc_user_id, log_date) WHERE qc_user_id IS NOT NULL`. It sits BESIDE `operator_id`, which references the **operators** master (shop-floor machinists) and therefore could never hold a login — that is exactly why the QC Call Register could only ever save the inspector's name. `operator_name` keeps its own snapshot: a signed-off inspection must not change when a person is renamed, moved or removed. **No schema change was needed for the GRN screen** — `goods_receipt_note_lines` has had both `qc_inspected_by` and `qc_inspected_by_text` for a long time; the form simply never asked who inspected, and three service paths stamped `user.id` (whoever SAVED the GRN, routinely a storekeeper). That half is a code fix plus a new form field. **ADDITIVE ONLY**: no existing row touched, nothing backfilled — the old text names were typed freely for years and a fuzzy match to logins would put guesswork in an audit trail. **Applied to the shared DB 2026-09-06** — verified live: `op_log.qc_user_id` present, uuid, nullable, sitting alongside `operator_id`/`operator_name`; `op_log_qc_user_idx` created; all 50 existing rows null (nothing backfilled, as intended); the GRN pair `qc_inspected_by`/`qc_inspected_by_text` confirmed already present; and a `select *` over op_log succeeds, which is what unblocks SO Overview and SO Status. |
| 2026-09-17 | `0132_final_inspection_qc_process.sql` (hand-written, idempotent, data-only) | **"Final Inspection" joins the QC Process Master** (ADR-168). Inserts one active `qc_processes` row `code = 'Final Inspection'` per company that has none (case-insensitive `NOT EXISTS`), `created_by`/`updated_by` = the company's oldest admin (as 0116). It is the name the server now writes as the terminal QC op under ADR-069 Rule B (was `DIR`); the JC QC picker lists active master rows only, so the row must exist. No table/column change; existing `jc_ops` named DIR are not rewritten. **Applied to TEST 2026-09-17**; PROD pending the user's go. |
| 2026-09-17 | `0133_production_orders.sql` (hand-written, idempotent) | **Production Orders** (ADR-170). Enum `store_txn_source_type` += `production_order_close`; `plans.ops_source text NOT NULL DEFAULT 'plan'` (+ CHECK `'plan'|'route_card'`); new table `production_orders` (code IN-PRO-#####, plan_id UNIQUE live, route_card_id + revision snapshot, job_card_id UNIQUE live, order_qty, target_date, status open/closed, closed_at/by, credited_qty; RLS company read / manager write); `job_cards.production_order_id uuid NULL → production_orders ON DELETE SET NULL` + partial index. No view changes. **Applied to TEST 2026-09-17**; PROD pending the user's go. |
| 2026-09-17 | `0134_item_procurement_type.sql` (hand-written, idempotent) | **Item Source make/buy** (ADR-171). `items.procurement_type text NOT NULL DEFAULT 'make'` + CHECK `'make'|'buy'` + partial index `(company_id, procurement_type)`. Buy items skip plan / route card / Production Order — the Planning line raises a standard PR against the SO line. **Applied to TEST 2026-09-17**; PROD pending the user's go. |
| 2026-09-18 | `0135_idempotency_keys.sql` (hand-written, idempotent) | **Idempotency keys** (ADR-172). Server-only table `idempotency_keys` (user_id, key, method, path, status_code, response_body jsonb, created_at, completed_at; unique (user_id, key); RLS enabled, no policies — API connects as postgres). A resent write is answered with the first run's stored result. **Applied to TEST 2026-09-18**; PROD pending the user's go. |
| 2026-09-21 | `0137_plan_customer_dispatch_date.sql` (hand-written, idempotent) | **Plan → Customer Dispatch Date** (ADR-174). `plans.customer_dispatch_date date` (nullable): the day the goods must leave for the customer, entered on the plan (defaulted from the SO line due date), editable while in planning. Read-only downstream: pre-fills the Production Order date (label now "Customer Dispatch Date"), shown on the Job Card header / list and on the Customer Dispatch pending lines (MIN over the SO line's plans). **Applied to TEST 2026-09-21**; PROD pending the user's go. |
| 2026-09-21 | `0139_task_management.sql` (hand-written, idempotent) | **Task Board → Inbox / Outbox / My To-Do / All Tasks** (ADR-176). `tasks` += `task_type text NOT NULL DEFAULT 'assigned'` ('assigned'/'personal'), `start_date date`, `reminder_at timestamptz`, `completed_at timestamptz`, `completed_by uuid → users`, `completion_remark text`; `due_date` **DROP NOT NULL** (personal to-dos may have none); partial index `(company_id, created_by)`. Enum `task_priority` += `'urgent'` BEFORE `'high'` (`medium` kept, labelled "Normal"). New append-only `task_history` (task_id FK cascade, action text, from_value, to_value, note, created_at/by; index (task_id, created_at); RLS company read + own-row insert). `file_registry.task_id uuid NULL → tasks ON DELETE SET NULL` + partial index — task attachments use `category = 'task'`. Overdue stays derived. Personal to-dos numbered `TODO-####`, assigned tasks `TSK-####`. **Applied to TEST 2026-09-21** (1 existing row kept as `assigned`); PROD pending the user's go. |
| 2026-09-25 | `0144_production_rejects_leave_available.sql` (hand-written, idempotent) | **Production rejects leave `available`** (ADR-183). Re-emits `v_jc_op_status` (0130 text, same columns): `available` / `pending_qty` (non-outsource) also subtract rejects on 'complete' rows less `LOG-NC-…` 'complete' re-injections; an all-rejected op reads `in_progress`. No table change. Rollback: re-run 0130. |
| 2026-09-26 | `0145_short_close_counts_credited.sql` (hand-written, idempotent, data only) | **ADR-184.** Fills `production_orders.lost_qty` on short-closed orders (NC loss on the order's card, capped at order − credited), closes the card + rework descendants of every short-closed order, re-stamps route-card plans now fully covered to `jc_created`. No DDL. **Applied to TEST 2026-09-26**; PROD pending the user's go. |
| 2026-09-26 | `0146_so_delete_guards.sql` (hand-written, idempotent) | **ADR-184.** `invoices.sales_order_id` FK becomes `ON DELETE RESTRICT` (was CASCADE), so a Sales Order can never be hard-deleted together with its invoices and payments. **Applied to TEST 2026-09-26**; PROD pending the user's go. |
| 2026-09-26 | `0149_o2c_accountability_links.sql` (hand-written, idempotent) | **ADR-185.** `store_issues` += `job_card_id uuid → job_cards ON DELETE SET NULL`, `production_order_id uuid → production_orders ON DELETE SET NULL` (+ partial index on job_card_id), backfilled from `ref_no`; set by the service on every issue that names a card / order. Trigger `store_transactions_immutable` (BEFORE UPDATE OR DELETE) refuses any change to a ledger row unless the session sets `innovic.ledger_maintenance = 'on'`. Rollback in the file header. |
| 2026-09-26 | `0150_design_one_time_log_client_payment_days.sql` (hand-written, idempotent) | **ADR-188.** (a) `design_work_log` += `design_tracker_id uuid → design_tracker ON DELETE SET NULL` (+ partial index where `deleted_at is null`) — the one engineer time log; Design Tracker "Log Time" now writes here. (b) Data move: every live `design_time_log` row copied into `design_work_log` (`engineer_text` = the login email of the one live same-company user whose email or full name matches `worker_text` case-insensitively, else `worker_text`; category `Design`, `design_project_id` = the SO's live Design Project when exactly one exists, else NULL, audit columns copied) and soft-deleted in `design_time_log` in the same statement; re-run safe. `design_time_log` kept, no longer written. (c) `clients` += `payment_days integer` (nullable, CHECK 0..365) — Payment Days, the default for a new invoice's Payment Terms. Rollback in the file header. |
| 2026-09-26 | `0151_po_short_close.sql` (hand-written, idempotent) | **ADR-189.** `purchase_orders` += `short_closed_at timestamptz`, `short_closed_by uuid → users`, `short_close_reason text` (CHECK all-or-none, reason non-blank): Short Close of an issued PO. `nc_register.job_card_id` DROP NOT NULL + CHECK `job_card_id IS NOT NULL OR grn_line_id IS NOT NULL`: Incoming QC raises an NC for a bought-material reject. Rollback in the file header. |
| 2026-09-26 | `0152_store_issue_reversal.sql` (hand-written, idempotent) | **ADR-189.** `store_issues` += `reversed_at timestamptz`, `reversed_by uuid → users`, `reversal_reason text`, `reversal_store_transaction_id uuid → store_transactions ON DELETE SET NULL` (CHECK who / when / why all-or-none, reason non-blank): a store issue is undone only by an opposite 'in' ledger entry and stays on the register marked Reversed. Rollback in the file header. |
|
