...
| 2026-09-26 | `0147_o2c_accountability_links.sql` (hand-written, idempotent) | **ADR-185.** `store_issues` += `job_card_id uuid → job_cards ON DELETE SET NULL`, `production_order_id uuid → production_orders ON DELETE SET NULL` (+ partial index on job_card_id), backfilled from `ref_no`; set by the service on every issue that names a card / order. Trigger `store_transactions_immutable` (BEFORE UPDATE OR DELETE) refuses any change to a ledger row unless the session sets `innovic.ledger_maintenance = 'on'`. Rollback in the file header. |
|
