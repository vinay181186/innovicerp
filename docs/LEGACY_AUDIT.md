...
| 9   | Sales (Entry)       | `dispatch`         | `renderDispatchRegister` (10711)         | —                                                          | ❌ Missing (`dispatchLog` collection in ADR-017 deferral)                                                                                                               |
...
| 31  | Design              | `bommaster`        | `renderBOMMaster` (8438)                 | `/bom-masters` (API done, web pending)                     | 🟡 API + cascade shipped 2026-05-20 (BOM-1..8 per ADR-028) — DB + service + 24 tests + sales-orders cascade hook live; web list/detail/forms pending                    |
| 32  | Design              | `designtracker`    | `renderDesignTracker` (7259)             | —                                                          | 📦 Deferred (ADR-022)                                                                                                                                                   |
| 33  | Design              | `routecards`       | `renderRouteCards` (10078)               | —                                                          | ❌ Missing (route cards = ordered op sequence template per item; currently embedded in JC ops, not a reusable master)                                                   |
...
| 71  | Finance             | `invoices`         | `renderInvoices` (21096)                 | —                                                          | ❌ Missing (sales invoices — distinct from SO; legacy has invoice register with payment tracking)                                                                       |
| 72  | Finance             | `socosting`        | `renderSOCosting` (17249)                | —                                                          | ❌ Missing (per-SO costing roll-up — material + labour + overhead, compares estimate vs actual)                                                                         |
| 73  | Finance             | `costcenters`      | `renderCostCenters` (17165)              | —                                                          | ❌ Missing (cost center master — distinct from departments)                                                                                                             |
| 74  | Finance             | `stockvaluation`   | `renderStockValuation` (20927)           | —                                                          | ❌ Missing (current stock × unit cost per item)                                                                                                                         |
